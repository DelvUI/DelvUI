...
namespace DelvUIPlugin.Interface {
    public class NinjaHudWindow : HudWindow {
        ...
        private new int BarWidth => 240;
        private new int XOffset => 120;
        ...
        private void NinkiBar() {
            var gauge = (float)PluginInterface.ClientState.JobGauges.Get<NINGauge>().Ninki;
            var barSize = new Vector2(BarWidth, 7);
            var cursorPos = new Vector2(CenterX - XOffset, CenterY + YOffset - 150);
        ...
        private void HutonText() {
            var gauge = PluginInterface.ClientState.JobGauges.Get<NINGauge>().HutonTimeLeft;
            var huton = gauge / 1000;
            var barSize = new Vector2(BarWidth, BarHeight);
            var cursorPos = new Vector2(CenterX - XOffset, CenterY + YOffset - 100);
            var drawList = ImGui.GetWindowDrawList();
            drawList.AddRectFilled(cursorPos, cursorPos + barSize, 0x88000000);
            drawList.AddRectFilledMultiColor(
            cursorPos, cursorPos + new Vector2(barSize.X * huton/70, barSize.Y), 
                0xFFCE503C, 0xFFCE503C, 0xFFCE503C, 0xFFCE503C
            ); 

            drawList.AddRect(cursorPos, cursorPos + barSize, 0xFF000000);
            DrawOutlinedText(huton.ToString(), new Vector2(cursorPos.X+barSize.X * huton/100-(huton==100?30:huton>5?20:0), cursorPos.Y+-2));
            //DrawOutlinedText(huton.ToString(), new Vector2(cursorPos.X, cursorPos.Y + -2));
