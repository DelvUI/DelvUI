...
- Implemented profiles. You can clone, export and import profiles.
...
- Added Experience Bar hud element.
- Added Pull timer hud element.
- Added option to show status effects with their original shape and dispellable indicator.
- Lots of improvements to the config window.
...
- Added party frames. Currently they support:
    ...
    + The game's party sorting settings. You can use the game's sorting options and change player's order in the social window, and it will be reflected in DelvUI's party frames.
