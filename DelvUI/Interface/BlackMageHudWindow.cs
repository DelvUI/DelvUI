...
namespace DelvUI.Interface
{
    public class BlackMageHudWindow : HudWindow
    {
        ...
        protected override void Draw(bool _) {
            ...
            if (ShowTripleCast) DrawTripleCast();

            if (ShowFirestarterProcs || ShowThundercloudProcs) DrawProcs();

            if (ShowDotTimer) DrawDotTimer();
        }
        ...
        protected virtual void DrawPolyglot() {
            ...
            var scale = gauge.NumPolyglotStacks == 2
                ? 0
                : gauge.IsEnoActive()
                    ? gauge.TimeUntilNextPolyglot / 30000f
                    : 1;

            scale = 1 - scale;

            var y = OriginY - ManaBarHeight - VerticalSpaceBetweenBars - UmbralHeartHeight - VerticalSpaceBetweenBars - PolyglotHeight;
            if (ShowTripleCast) y = y - VerticalSpaceBetweenBars - TripleCastHeight;
        ...
        protected virtual void DrawProcs() {
            ...
            if (firestarterTimer == 0 && thundercloudTimer == 0) return;
