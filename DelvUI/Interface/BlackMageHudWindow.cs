...
namespace DelvUI.Interface
{
    public class BlackMageHudWindow : HudWindow
    {
        ...
        {
            BLMGauge gauge = PluginInterface.ClientState.JobGauges.Get<BLMGauge>();

            PlayerCharacter actor = PluginInterface.ClientState.LocalPlayer;
            Vector2 barSize = _config.ManaBarSize;
            Vector2 cursorPos = new Vector2(OriginX - barSize.X / 2, OriginY - barSize.Y);

            Dictionary<string, uint> color = gauge.InAstralFire()
                ? _config.ManaBarFireColor.Map
                : gauge.InUmbralIce()
                    ? _config.ManaBarIceColor.Map
                    : _config.ManaBarNoElementColor.Map;

            BarBuilder builder = BarBuilder.Create(OriginX - barSize.X / 2, OriginY - barSize.Y, (int) barSize.Y, (int) barSize.X);
            ...
            {
                Vector2 position = new Vector2(OriginX - barSize.X / 2 + _config.ManaThresholdValue / 10000f * barSize.X, cursorPos.Y + barSize.Y);
                Vector2 size = new Vector2(3, barSize.Y);
        ...
        protected virtual void DrawUmbralHeartStacks()
        {
            ...
            Vector2 cursorPos = new Vector2(OriginX - totalWidth / 2, OriginY - _config.ManaBarSize.Y - _config.Padding.Y - _config.UmbralHeartSize.Y);

            Bar bar = BarBuilder.Create(cursorPos.X, cursorPos.Y, _config.UmbralHeartSize.Y, totalWidth)
                                .SetChunks(3)
        ...
        protected virtual void DrawPolyglot()
        {
            ...
            // 1
            BarBuilder builder = BarBuilder.Create(OriginX - totalWidth / 2, y, _config.PolyglotSize.Y, _config.PolyglotSize.X)
        ...
        {
            StatusEffect tripleStackBuff = PluginInterface.ClientState.LocalPlayer.StatusEffects.FirstOrDefault(o => o.EffectId == 1211);
            ...
            Bar bar = BarBuilder.Create(cursorPos.X, cursorPos.Y, _config.TripleCastSize.Y, totalWidth)
                                .SetChunks(3)
                                .SetChunkPadding(_config.Padding.X)
        ...
        protected virtual void DrawProcs()
        {
            ...
            {
                Vector2 position = new Vector2(x, y - totalHeight / 2f);
            ...
            {
                Vector2 position = new Vector2(x, firestarterTimer == 0 ? y - totalHeight / 2f : y + _config.Padding.Y / 2f);
        ...
        protected virtual void DrawDotTimer()
        {
            ...
            Vector2 position = new Vector2(x, y + _config.DotTimerHeight / 2f);
        ...
        {
            ImDrawListPtr drawList = ImGui.GetWindowDrawList();
            Vector2 size = new Vector2((_config.ManaBarSize.X / 2f - _config.PolyglotSize.X - _config.Padding.X * 2f) * scale, height);
