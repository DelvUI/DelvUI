...
namespace DelvUI.Interface
{
    public class ScholarHudWindow : HudWindow
    {
        ...
        protected override void Draw(bool _) {
            if (ShowFairyBar) DrawFairyBar();

            if (ShowBioBar) DrawBioBar();

            if (ShowAetherBar) DrawAetherBar();
        }
        ...
        private void DrawFairyBar() {
            ...
            DrawOutlinedText(
                gauge.ToString(CultureInfo.InvariantCulture), new Vector2(
                    cursorPos.X + BarSize.X * gauge / 100 - (gauge == 100
                        ? 30
                        : gauge > 3
                            ? 20
                            : 0), cursorPos.Y + BarSize.Y / 2 - 12
                )
            );
        }
        ...
        private void DrawBioBar() {
            ...
            drawList.AddRect(cursorPos, cursorPos + BarSize, 0xFF000000);
            DrawOutlinedText(
                bioDuration.ToString(CultureInfo.InvariantCulture), new Vector2(
                    cursorPos.X + BarSize.X * bioDuration / 30 - (bioDuration == 30
                        ? 30
                        : bioDuration > 3
                            ? 20
                            : 0), cursorPos.Y + BarSize.Y / 2 - 12
