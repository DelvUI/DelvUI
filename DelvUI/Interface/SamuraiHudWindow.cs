...
namespace DelvUI.Interface
{
    public class SamuraiHudWindow : HudWindow
    {
        ...
        protected override void Draw(bool _) {
            if (GaugeEnabled) DrawKenkiBar();

            if (SenEnabled) DrawSenResourceBar();

            if (MeditationEnabled) DrawMeditationResourceBar();

            if (HiganbanaEnabled) DrawHiganbanaBar();

            if (BuffsEnabled) DrawActiveBuffs();
        }
